    ...
    for i in  range(episodes):
        ...
        for (usable_ace, player_sum, dealer_card), action in player_trajectory:
            if action == target_policy_player(usable_ace, player_sum, dealer_card):
                ...
                numerator = 0.0
                break
        importance_sampling_raito = numerator/denominator
        importance_sampling_raitos.append(importance_sampling_raito)

        returns.append(reward)

    importance_sampling_raitos = np.asarray(importance_sampling_raitos)
    returns = np.asarray(returns)

    weighted_returns = importance_sampling_raitos * returns
    weighted_returns = np.add.accumulate(weighted_returns)
    importance_sampling_raitos = np.add.accumulate(importance_sampling_raitos)

    ordinary_estimates = weighted_returns / np.arange(1, episodes + 1)

    with np.errstate(divide='ignore', invalid='ignore'):
        weighted_estimates = np.where(importance_sampling_raitos != 0, weighted_returns / importance_sampling_raitos, 0)
